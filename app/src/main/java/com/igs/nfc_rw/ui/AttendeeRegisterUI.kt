...
@Composable
fun AttendeeRegisterUI(navController: NavController, nfcReader: NFCReader? = null) {
    val padding = 30f.dp
    val loggerHead = "AttendeeRegisterUI"
    var firstName by remember { mutableStateOf("") }
    var lastName by remember { mutableStateOf("") }
    var id by remember { mutableStateOf(nfcReader?.lastReadingTagId ?: "") }
    var firstNameError by remember { mutableStateOf(true) }
    var lastNameError by remember { mutableStateOf(true) }

    // Update ID when lastReadingTagId changes
    if (nfcReader != null) {
        LaunchedEffect(nfcReader.lastReadingTagId) {
            id = nfcReader.lastReadingTagId ?: ""
        }
    }
    ...
    Column(
        ...
        Text(
            ...
            text = stringResource(R.string.visitor_register_title),
            style = MaterialTheme.typography.headlineMedium
        )
        Text(
            modifier = Modifier.padding(top = padding),
            text = stringResource(R.string.visitor_register_description),
